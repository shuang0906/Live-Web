...
function assignRandomFromPool(pool, assigned, socketId, emitEventName) {
    if (pool.length > 0) {
        const index = Math.floor(Math.random() * pool.length);
        const item = pool[index];
        assigned[socketId] = item;
        ...
        // Handle the case where there are no available items (e.g., assign a default item or reject connection)
    }
}
...
io.sockets.on('connection', function (socket) {
    ...
    assignRandomFromPool(availableColors, userColors, socket.id, 'assign color');
    assignRandomFromPool(availableNames, userNames, socket.id, 'assign username');
