...
function setupAndDrawCanvas(canvasId, dotDatas) {
    ...
    // 	const newZoomLevel = Math.min(5, Math.max(1, zoomLevel * scaleFactor));
    ...
    function zoom(event) {
        ...
        const rect = canvas.getBoundingClientRect();
        const position = {
            x: Math.round((event.clientX - rect.left) * 100) / 100 / newScale,
            y: Math.round((event.clientY - rect.top) * 100) / 100 / newScale
        };
