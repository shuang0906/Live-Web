...
io.sockets.on('connection', function (socket) {
	...
	socket.on('cursor move', (data) => {
		...
		io.to('some room').emit('cursor update', {
			socketId: socket.id,
	...
	socket.on('cursor leave', (data) => {
		io.to('some room').emit('remove cursor', {
			socketId: socket.id,
	...
	socket.on('connect dot', (data) => {
		...
		// });
		db.update(
			{ canvasId: data.canvasId },  // Find the correct canvas document
			{ 
				$push: { connections: updatedData }  // Append new connection to the array
			}, 
			{ upsert: true }, // Create document if it doesn't exist
			function (err, numAffected, affectedDocuments, upsert) {
				if (err) console.error("Database error:", err);
				console.log("Database updated:", affectedDocuments);
			}
		);
	});
