cd liveweb
weblive
cd weblive
npm install socket.io
apt-get update
install nodejs
apt-get install nodejs
apt-get install npm
ls
cd liveweb
npm install socket.io
npm install express
node server.js
ls
cd liveweb
node sercer.js
node server.js
cd liveweb
node server.js
cd liveweb
node server.js
cd ../
cd liveweb-week2
node server.js
cd liveweb
npm server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
cd weblive-week2
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
tops
htop
killall node
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
ls
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
killall node
node sever.js
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
run server.js
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
server.js
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
cd liveweb-week2
node server.js
killall node
node server.js
cd liveweb-week2
node server.js
cd liveweb-week3
node server.js
../
cd
cd liveweb-week2
node server.js
cd
npm install express
cd liveweb-week3
npm install express
npm install socket
npm install socket.io
node server.js
apt-get install certbot
certbot certonly --standalone
node server.js
killall node
node server.js
cd live-web3
cd liveweb-week3
node server.js
killall node
node server.js
cd liveweb-week3
node server.js
killall node
node server.js
apt-get install certbot
killall node
liveweb-week2
cd liveweb-week2
apt-get install certbot
certbot certonly --standalone
cd liveweb-week2
certbot certonly --standalone
cd liveweb-week3
node server.js
cd liveweb-week3
node server.js
killall node
node server.js
../
..
node server.js
killall node
node server.js
killall node
node sever.js
node server.js
killall node
node server.js
..
cd liveweb-week2
node server.js
cd..
cd ..
cd liveweb-week3
node server.js
cd liveweb-week3
node server.js
killall node
node server.js
killall node
node server.js
killall node
node server.js
cd liveweb-week3
node server.js
killall node
node server.js
cd liveweb-week3
node server.js
cd liveweb-week3
node server.js
node app.js
node server.js
cd livewen-week3
cd liveweb-week3
$ npm install --save socketio-file-upload
..
../
npm install --save socketio-file-upload
cd liveweb-week3
$ npm install --save socketio-file-upload
npm install --save socketio-file-upload
node server.js
killall node
node server.js
cd siodu
cd siofu
node server.js
cd liveweb-week3
node server.js
cd liveweb-midterm
npm install nedb
cd liveweb-midterm
node server.js
cd liveweb-week2
node server.js
cd liveweb-dot
node server.js
npm intall nedb
npm install nedb
cd liveweb-midterm
npm install nedb
node server.js
cd liveweb-dot
npm install nedb
node server.js
cd liveweb-dot
node server.js
cd ..
cd liveweb-midterm
node server.js
kilall node
killall node
node server.js
cd ..
cd liveweb-dot
node server.js
killall node
node server.js
cd..
cd ..
killall node
cd liveweb-dot
node server,js
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
SIGINT
kill
killall node
cd liveweb-dot
node server.js
killall node
node server.js
killall node
node server,js
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
npm install ef-infinite-canvas
node server.js
killall node
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
ls -l server.js
node -v
node server.js --verbose
whoami
node server.js
killall node
node server.js
npm install
node server.js
ls -l server.js
node server.js --trace-warnings
cd liveweb-dot
node server.js
cd liveweb-dot
node server.js
killall node
node server.js
cd liveweb-dot
node server.js
ls
cat server.js
node server.js
cd liveweb-dot
node server.js
killall node
node server.js
../
cd..
cd ..
cd liveweb-dot
cat server.js
node server.js
cd 2025
cd connect-the-dot
cs connect-the-dot
cd connect-the-dot 
cd connectthedot 
fr connectthedot 
mkdir connect-the-dot
cd connect-the-dot
node server.js
taskkill /PID <PID> /F
sudo lsof -i :80
sudo kill -9 17362
node server.js
cd liveweb-dot
node server.js
cd 2025
cd connect-the-dot
node server.js
cd liveweb-week2
node server.js
killall node
cd liveweb-week2
node server.js
cd 2025
cd connect-the-dot
node server.js
cd liveweb-dot
node server.js
cd ..
cd liveweb-week2
node server.js
cd ..
cd liveweb-dot
node server.js
cd ..
cd 2025
cd connect-the-dot
node server.js
Error: Cannot find module 'socketio-file-upload'
node server.js
cd ..
cd..
cd ..
sudo apt update
sudo apt install certbot
nginx -v
apache2 -v
sudo certbot certonly --standalone -d creativecoding.shunhuang.work
ls -l /etc/letsencrypt/live/creativecoding.shunhuang.work/
sudo crontab -e
mkdir -p /etc/ssl/mycerts
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/mycerts/selfsigned.key -out /etc/ssl/mycerts/selfsigned.crt
mkdir -p /etc/ssl/mycerts
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/mycerts/selfsigned.key -out /etc/ssl/mycerts/selfsigned.crt
cd 2023
cd 2025
cd connect-the-dot
node server.js
sudo certbot certificates
cd 2025
cd connect-the-dot
node server.js
cd 2025
cd connect-the-dot
node server.js
node server,js
node server.js
cd liveweb-dot
cd 2025
cd connect-the-dot
node server.js
cd ..
cd 2025
cd connect-the-dot
node server.js
cd 2025
cd connect-the-dot
node server.js
killall node
cd 2025
cd connecct-the-dot
cd connect-the-dot
node server.js
cd 2025
cd connect-the-dot
node server.js
git init
git remote add origin git@github.com:shuang0906/Live-Web.git
git add .
git commit -m "Initial commit"
git config --global user.name "shuang0906"
git config --global user.email "shunhuang0906@gmail.com"
git config --global --list
git add .
git commit -m "Initial commit"
git branch -M main  # Rename the default branch to 'main'
git push -u origin main
ls -al ~/.ssh
ssh-keygen -t ed25519 -C "shunhuang0906@gmail.com"
eval "$(ssh-agent -s)"
ssh-add ~/.ssh/id_ed25519
cat ~/.ssh/id_ed25519.pub
ssh -T git@github.com
git config --global user.name "shuang0906"
ssh -T git@github.com
ssh -vT git@github.com
cat ~/.ssh/id_ed25519.pub
ssh -T git@github.com
eval "$(ssh-agent -s)"  # Start SSH agent
ssh-add ~/.ssh/id_ed25519
ssh -T git@github.com
nano ~/.ssh/config
ssh -T git@github.com
git init
git add -A
git commit -m 'Added my project'
git remote add origin git@github.com:shuang0906/Live-Web.git
git push -u -f origin main
cat ~/.ssh/id_ed25519.pub
ssh -T git@github.com
git push -u -f origin main
cd 2025
cd connect-the-dot
node server.js
